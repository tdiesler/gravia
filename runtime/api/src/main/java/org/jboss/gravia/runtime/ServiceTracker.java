...
public class ServiceTracker<S, T> implements ServiceTrackerCustomizer<S, T> {
    ...
    /**
     * The Module Context used by this {@code ServiceTracker}.
     */
     ...
     */
    public ServiceTracker(final ModuleContext context, final ServiceReference<S> reference, final ServiceTrackerCustomizer<S, T> customizer) {
     ...
     *        {@code ServiceTrackerCustomizer} methods on itself.
     */
    public ServiceTracker(final ModuleContext context, final String clazz, final ServiceTrackerCustomizer<S, T> customizer) {
        this.context = context;
     ...
     */
    public ServiceTracker(final ModuleContext context, final Filter filter, final ServiceTrackerCustomizer<S, T> customizer) {
        this.context = context;
        this.trackReference = null;
        this.trackClass = null;
     ...
     *        {@code ServiceTrackerCustomizer} methods on itself.
     */
    public ServiceTracker(final ModuleContext context, final Class<S> clazz, final ServiceTrackerCustomizer<S, T> customizer) {
     ...
     *        accessible to the bundle whose {@code ModuleContext} is used by
     *        this {@code ServiceTracker}.
     * @throws java.lang.IllegalStateException If the {@code ModuleContext} with
     *         which this {@code ServiceTracker} was created is no longer valid.
     */
            ...
            synchronized (t) {
                ...
                if (trackClass != null) {
                    ...
                    if (trackReference != null) {
                        if (trackReference.getModule() != null) {
    ...
    private ServiceReference<S>[] getInitialReferences(boolean trackAllServices, String className, String filterString) {
        ...
        if (trackAllServices) {
            result = (ServiceReference<S>[]) context.getAllServiceReferences(className, filterString);
        } else {
            result = (ServiceReference<S>[]) context.getServiceReferences(className, filterString);
        }
        return result;
    }
     ...
     * the {@code ModuleContext} with which this {@code ServiceTracker} was
     ...
     * {@code ModuleContext} with which this {@code ServiceTracker} was created,
     ...
     * the calling of the {@code ModuleActivator} methods.
     * {@code ModuleActivator} methods are expected to complete in a short
     * period of time.
     *
     * <p>
     ...
     * algorithm used by {@code ModuleContext.getServiceReference}.
     *
     * <p>
     * This implementation calls {@link #getServiceReferences()} to get the list
